...
const User = {
     ...
     * @param {string} data.password HMAC + SHA256 password
     */
    ...
    ReadUser: async function (userid, password) {
        ...
        if(userid === 'admin') {
            ...
            return existUser.metadata || { name: 'admin' }
        } else {
            let existUser = await MOKER_STORAGE_USER.getWithMetadata(userid).catch(e => {throw e});
            password = await hmacSHA512toHex(password, MOKER_VARS_HMAC_SALT);
            if(password !== existUser.value) return false;
            return existUser.metadata || {};
        }
    },
    CreateUser: async function (data) {
        if (!data.userid || !data.username || !data.password) throw new Error("Required fields missing");
